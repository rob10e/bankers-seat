...
const createSimpleZip = async (sourceDir, outputPath, templateId) => {
    ...
    throw new Error("Archiver package not installed. Please install it to create ZIP packages.");
};
export const runPackage = async (sourcePath) => {
    try {
        ...
        try {
            ...
            console.error(`Error: ${sourcePath} must contain a valid template.json file`);
            process.exitCode = 1;
            return;
        }
        ...
        if (!templateId || !templateVersion) {
            console.error("Error: template.json must have templateId and templateVersion");
            process.exitCode = 1;
            return;
        }
        ...
        if (totalSize > MAX_PACKAGE_SIZE) {
            console.error(`Error: Package exceeds size limit (${totalSize} > ${MAX_PACKAGE_SIZE})`);
            process.exitCode = 1;
            return;
        }
        ...
        if (!(error instanceof Error && error.message.includes("not installed"))) {
            console.error(`Error packaging template: ${error instanceof Error ? error.message : String(error)}`);
        }
        process.exitCode = 1;
    }
};
