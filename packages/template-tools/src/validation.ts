...
type GameTemplate = {
  ...
  currency: { fractionDigits: number };
  bank: {
...
const toIssueFromAjv = (error: ErrorObject): TemplateValidationIssue => {
  return {
...
const collectDistinctDuplicates = <T>(
  values: T[],
): Array<{ value: T; indexes: number[] }> => {
...
const extractFieldOperations = (
  operation: TemplateOperation,
  path: string,
): Array<{ operation: TemplateOperation; path: string }> => {
  if (operation.type === "composite" && Array.isArray(operation.steps)) {
    return operation.steps.flatMap((step, stepIndex) =>
      extractFieldOperations(step as TemplateOperation, `${path}/steps/${stepIndex}`),
    );
  }
...
const validateAssetPathValue = (
  ...
  if (
    ...
    addIssue(
      ...
      "asset-absolute-path",
      issuePath,
      "Asset path must be relative to the template directory.",
    );
    return;
  }
  ...
  if (normalizedCandidate.split("/").includes("..")) {
    addIssue(
      issues,
      "asset-parent-traversal",
      issuePath,
      "Asset path must not include parent-directory traversal segments.",
    );
    return;
  }
  ...
  if (isOutside) {
    addIssue(
      issues,
      "asset-outside-template",
      issuePath,
      "Asset path resolves outside the template directory.",
    );
  }
  ...
  if (!allowedAssetExtensions.has(extension)) {
    addIssue(
      issues,
      "asset-extension-not-allowed",
      issuePath,
      "Asset extension must be one of: .png, .jpg, .jpeg, .webp, .svg.",
    );
  }
};

const semanticValidateTemplate = (
  ...
  if (template.playerCount.minimum > template.playerCount.maximum) {
    addIssue(
      issues,
      "player-count-range-invalid",
      "/playerCount",
      "playerCount.minimum must be less than or equal to playerCount.maximum.",
    );
  }
  ...
  if (template.currency.fractionDigits !== 0) {
    addIssue(
      issues,
      "currency-fraction-digits-invalid",
      "/currency/fractionDigits",
      "fractionDigits must be 0 because money is stored as integer base units.",
    );
  }
  ...
  if (!template.bank.allowPlayerOverdraft && template.bank.startingPlayerBalance < 0) {
    addIssue(
      issues,
      "starting-balance-overdraft-conflict",
      "/bank/startingPlayerBalance",
      "startingPlayerBalance cannot be negative when allowPlayerOverdraft is false.",
    );
  }
  ...
  collectDistinctDuplicates(denominationValues).forEach(({ value, indexes }) => {
    indexes.forEach((index) => {
      addIssue(
        ...
        `Duplicate denomination value '${String(value)}'.`,
      );
    });
  });
  ...
  collectDistinctDuplicates(fieldIds).forEach(({ value, indexes }) => {
    indexes.forEach((index) => {
      addIssue(
        issues,
        "duplicate-player-field-id",
        `/playerFields/${index}/id`,
        `Duplicate player field id '${value}'.`,
      );
    });
  });
  ...
  (template.playerFields ?? []).forEach((field, fieldIndex) => {
    ...
    if (!optionValues.includes(String(field.default))) {
      addIssue(
        issues,
        "enum-default-not-found",
        `/playerFields/${fieldIndex}/default`,
        `Enum default '${String(field.default)}' is not present in options.`,
      );
    }
  });
  ...
  (template.actions ?? []).forEach((action, actionIndex) => {
    ...
    fieldOps.forEach(({ operation, path }) => {
      const fieldId = String((operation as { fieldId?: unknown }).fieldId ?? "");
      const field = definedFieldMap.get(fieldId);
      if (!field) {
        addIssue(
          issues,
          "action-field-reference-missing",
          `${path}/fieldId`,
          `Operation references missing field '${fieldId}'.`,
        );
        return;
      }
  ...
  const assetsToValidate: Array<{ path: string; issuePath: string }> = [
  ...
  (template.denominations ?? []).forEach((entry, index) => {
    const candidate = (entry as { asset?: string }).asset;
    if (!candidate) {
      return;
    }
  ...
  (template.playerFields ?? []).forEach((field, index) => {
    ...
    if (!assetKeys.has(field.iconAssetKey)) {
      addIssue(
        issues,
        "icon-asset-key-not-found",
        `/playerFields/${index}/iconAssetKey`,
        `iconAssetKey '${field.iconAssetKey}' is not defined in assets.images.`,
      );
    }
  });
...
export const discoverTemplateFiles = async (rootDir: string): Promise<string[]> => {
...
export const validateTemplateObject = (
  template: unknown,
  ...
  const semanticIssues = semanticValidateTemplate(template as GameTemplate, templateDir);
...
export const validateTemplateFile = async (
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to parse template JSON file.";
    return {
      valid: false,
      issues: [
        {
